...
var DocumentParser = (function () {
        ...
        xmlUtil.foreach(xnums, function (n) {
            switch (n.localName) {
                ...
                case "num":
                    ...
                    var abstractNumId = xmlUtil.elementStringAttr(n, "abstractNumId", "val");
    ...
    DocumentParser.prototype.parseParagraphProperties = function (elem, paragraph) {
        ...
        this.parseDefaultProperties(elem, paragraph.cssStyle = {}, null, function (c) {
            ...
            switch (c.localName) {
                case "pStyle":
                    utils.addElementClass(paragraph, xmlUtil.className(c, "val"));
                    break;
                case "cnfStyle":
    ...
    DocumentParser.prototype.parseRunProperties = function (elem, run) {
        this.parseDefaultProperties(elem, run.cssStyle = {}, null, function (c) {
            switch (c.localName) {
                case "rStyle":
                    run.className = xmlUtil.className(c, "val");
                    break;
                case "vertAlign":
                    switch (xml_parser_1.default.attr(c, "val")) {
                        case "subscript":
    ...
    DocumentParser.prototype.parseDrawingWrapper = function (node) {
        ...
        for (var _i = 0, _b = xml_parser_1.default.elements(node); _i < _b.length; _i++) {
            ...
            switch (n.localName) {
                case "simplePos":
                    if (simplePos) {
                        posX.offset = xmlUtil.sizeAttr(n, "x", SizeType.Emu);
                        posY.offset = xmlUtil.sizeAttr(n, "y", SizeType.Emu);
                    }
                    break;
                case "extent":
                    result.cssStyle["width"] = xmlUtil.sizeAttr(n, "cx", SizeType.Emu);
                    result.cssStyle["height"] = xmlUtil.sizeAttr(n, "cy", SizeType.Emu);
                    break;
                case "positionH":
                case "positionV":
                    if (!simplePos) {
                        ...
                        if (offsetNode)
                            pos.offset = xmlUtil.sizeValue(offsetNode, SizeType.Emu);
                    }
                    break;
        ...
        for (var _i = 0, _a = xml_parser_1.default.elements(xfrm); _i < _a.length; _i++) {
            ...
            switch (n.localName) {
                case "ext":
                    ...
                    break;
                case "off":
                    result.cssStyle["left"] = xmlUtil.sizeAttr(n, "x", SizeType.Emu);
                    result.cssStyle["top"] = xmlUtil.sizeAttr(n, "y", SizeType.Emu);
                    break;
            }
        }
        return result;
    ...
    DocumentParser.prototype.parseTableColumns = function (node) {
        ...
        xmlUtil.foreach(node, function (n) {
            switch (n.localName) {
                case "gridCol":
                    result.push({ width: xmlUtil.sizeAttr(n, "w") });
                    break;
            }
        });
        return result;
    ...
    DocumentParser.prototype.parseTableProperties = function (elem, table) {
        ...
        this.parseDefaultProperties(elem, table.cssStyle, table.cellStyle, function (c) {
            switch (c.localName) {
                case "tblStyle":
                    utils.addElementClass(table, xmlUtil.className(c, "val"));
                    break;
                case "tblLook":
    ...
    DocumentParser.prototype.parseTablePosition = function (node, table) {
        var topFromText = xmlUtil.sizeAttr(node, "topFromText");
        var bottomFromText = xmlUtil.sizeAttr(node, "bottomFromText");
        var rightFromText = xmlUtil.sizeAttr(node, "rightFromText");
        var leftFromText = xmlUtil.sizeAttr(node, "leftFromText");
        ...
        xmlUtil.foreach(elem, function (c) {
            switch (c.localName) {
                ...
                case "color":
                    ...
                    break;
                case "sz":
                    style["font-size"] = style["min-height"] = xmlUtil.sizeAttr(c, "val", SizeType.FontSize);
                    break;
                case "shd":
    ...
    DocumentParser.prototype.parseIndentation = function (node, style) {
        var firstLine = xmlUtil.sizeAttr(node, "firstLine");
        var hanging = xmlUtil.sizeAttr(node, "hanging");
        var left = xmlUtil.sizeAttr(node, "left");
        var start = xmlUtil.sizeAttr(node, "start");
        var right = xmlUtil.sizeAttr(node, "right");
        var end = xmlUtil.sizeAttr(node, "end");
        if (firstLine)
    ...
    DocumentParser.prototype.parseSpacing = function (node, style) {
        var before = xmlUtil.sizeAttr(node, "before");
        var after = xmlUtil.sizeAttr(node, "after");
    ...
    DocumentParser.prototype.parseTrHeight = function (node, output) {
        switch (xml_parser_1.default.attr(node, "hRule")) {
            case "exact":
                output["height"] = xmlUtil.sizeAttr(node, "val");
                break;
            case "atLeast":
            default:
                output["height"] = xmlUtil.sizeAttr(node, "val");
...
var xmlUtil = (function () {
    ...
    xmlUtil.elementStringAttr = function (elem, nodeName, attrName) {
        var n = xml_parser_1.default.element(elem, nodeName);
        return n ? xml_parser_1.default.attr(n, attrName) : null;
    };
    ...
    xmlUtil.sizeAttr = function (node, attrName, type) {
        if (type === void 0) { type = SizeType.Dxa; }
        return xmlUtil.convertSize(xml_parser_1.default.attr(node, attrName), type);
    };
    xmlUtil.sizeValue = function (node, type) {
        if (type === void 0) { type = SizeType.Dxa; }
        return xmlUtil.convertSize(node.textContent, type);
    };
    xmlUtil.convertSize = function (val, type) {
        if (type === void 0) { type = SizeType.Dxa; }
    ...
    xmlUtil.className = function (node, attrName) {
        var val = xml_parser_1.default.attr(node, attrName);
...
var values = (function () {
    ...
    values.valueOfBold = function (c) {
        return xml_parser_1.default.boolAttr(c, "val", true) ? "bold" : "normal";
    };
    values.valueOfSize = function (c, attr) {
        ...
        return xmlUtil.sizeAttr(c, attr, type);
    };
    values.valueOfStrike = function (c) {
        return xml_parser_1.default.boolAttr(c, "val", true) ? "line-through" : "none";
    };
    values.valueOfMargin = function (c) {
        return xmlUtil.sizeAttr(c, "w");
    };
    values.valueOfBorder = function (c) {
        ...
        var size = xmlUtil.sizeAttr(c, "sz", SizeType.Border);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.mergeDeep = exports.isObject = exports.blobToBase64 = exports.keyBy = exports.resolvePath = exports.splitPath = exports.appendClass = exports.addElementClass = void 0;
...
function appendClass(classList, className) {
    return classList ? "".concat(classList, " ").concat(className) : className;
}
